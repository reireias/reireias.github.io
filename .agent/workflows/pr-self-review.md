...
7. **CI ステータス**: `env gh pr checks` の結果がすべて pass になっているか？ pending や fail がある場合は NG として報告すること。
...
2. **実装の妥当性** のレビューでは、変更ファイルの詳細を確認する。`git diff origin/master...HEAD -- <ファイルパス>` で個別ファイルの diff を取得して確認すること。
3. すべてのレビュー観点を評価し、結果を報告する。
